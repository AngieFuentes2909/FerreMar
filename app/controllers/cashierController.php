<?php
	...
	class cashierController extends mainModel{
		...
		public function registrarCajaControlador(){
            ...
            // Sanitizar formato de moneda colombiano (ej: 100.000 o 100.000,00 -> 10000)
            $efectivo = str_replace(' ', '', $efectivo);
            if (strpos($efectivo, ',') !== false) {
                $pos = strrpos($efectivo, ',');
                $efectivo = substr_replace($efectivo, '[DECIMAL]', $pos, 1);
            }
            $efectivo = str_replace('.', '', $efectivo);
            $efectivo = str_replace('[DECIMAL]', '.', $efectivo);
				...
				foreach($datos as $rows){
					$tabla.='
						<tr class="has-text-centered" >
							...
							<td>'.MONEDA_SIMBOLO.number_format($rows['caja_efectivo'], MONEDA_DECIMALES, MONEDA_SEPARADOR_DECIMAL, MONEDA_SEPARADOR_MILLAR).' '.MONEDA_NOMBRE.'</td>
			                <td>
