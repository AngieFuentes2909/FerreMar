<?php
	...
	class productController extends mainModel{
		...
		public function registrarProductoControlador(){
            ...
            // Sanitizar formato de moneda colombiano (ej: 25.000 o 25.000,00 -> 25000)
            $precio_compra = str_replace(' ', '', $precio_compra);
            if (strpos($precio_compra, ',') !== false) {
                $pos = strrpos($precio_compra, ',');
                $precio_compra = substr_replace($precio_compra, '[DECIMAL]', $pos, 1);
            }
            $precio_compra = str_replace('.', '', $precio_compra);
            $precio_compra = str_replace('[DECIMAL]', '.', $precio_compra);

            $precio_venta = str_replace(' ', '', $precio_venta);
            if (strpos($precio_venta, ',') !== false) {
                $pos = strrpos($precio_venta, ',');
                $precio_venta = substr_replace($precio_venta, '[DECIMAL]', $pos, 1);
            }
            $precio_venta = str_replace('.', '', $precio_venta);
            $precio_venta = str_replace('[DECIMAL]', '.', $precio_venta);
		                ...
		                <div class="media-content">
		                    <div class="content">
		                        <p>
		                            ...
		                            <strong>PRECIO:</strong> '.MONEDA_SIMBOLO.number_format($rows['producto_precio_venta'], MONEDA_DECIMALES, MONEDA_SEPARADOR_DECIMAL, MONEDA_SEPARADOR_MILLAR).' '.MONEDA_NOMBRE.', 
