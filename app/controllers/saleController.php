<?php
	...
	class saleController extends mainModel{
        ...
        public function buscarCodigoVentaControlador(){
            ...
            /*== Recuperando termino de busqueda y categoria ==*/
            ...
            if($producto != ""){
                $conditions[] = "(producto_nombre LIKE '%$producto%' OR producto_marca LIKE '%$producto%' OR producto_modelo LIKE '%$producto%' OR producto_codigo LIKE '%$producto%')";
            }
            ...
            $query .= " ORDER BY producto_nombre ASC LIMIT 100";

            /*== Seleccionando productos en la DB ==*/
            $datos_productos=$this->ejecutarConsulta($query);

            if($datos_productos->rowCount()>=1){

				$datos_productos=$datos_productos->fetchAll();

				$tabla='<div class="table-container mb-6" style="max-height: 380px; overflow-y: auto;"><table class="table is-striped is-narrow is-hoverable is-fullwidth">
                <thead>
				...
				foreach($datos_productos as $rows){
                    $stock_badge = ($rows['producto_stock_total'] > 5) ? '<span class="tag is-success" style="font-weight: 700;">'.$rows['producto_stock_total'].'</span>' : '<span class="tag is-danger" style="font-weight: 700;">'.$rows['producto_stock_total'].'</span>';
					$tabla.='
					<tr class="has-text-left" style="font-size: 12px; vertical-align: middle;">
                        <td><i class="fas fa-box fa-fw text-primary" style="color: #f97316;"></i> &nbsp; <strong>'.$rows['producto_nombre'].'</strong></td>
                        <td><code>'.$rows['producto_codigo'].'</code></td>
                        <td>'.$stock_badge.'</td>
                        <td>'.MONEDA_SIMBOLO.number_format($rows['producto_precio_venta'], MONEDA_DECIMALES, MONEDA_SEPARADOR_DECIMAL, MONEDA_SEPARADOR_MILLAR).'</td>
                        <td class="has-text-centered">
                            <button type="button" class="button is-link is-rounded is-small" style="background: linear-gradient(135deg, #f97316 0%, #ff5e36 100%) !important; border:none;" onclick="agregar_codigo(\''.$rows['producto_codigo'].'\')"><i class="fas fa-plus-circle"></i></button>
                        </td>
                    </tr>
                    ';
				}
				...
				return '<article class="message is-warning mt-4 mb-4">
				    ...
				    <div class="message-body has-text-centered">
						...
						No hemos encontrado ningún producto que coincida con tu búsqueda.
				    </div>
				</article>';
			}
        }
        ...
        public function buscarClienteVentaControlador(){
            ...
            $query = "SELECT * FROM cliente WHERE cliente_id!='1'";
            
            if($cliente != ""){
                $query .= " AND (cliente_numero_documento LIKE '%$cliente%' OR cliente_nombre LIKE '%$cliente%' OR cliente_apellido LIKE '%$cliente%' OR cliente_telefono LIKE '%$cliente%')";
            }
            
            $query .= " ORDER BY cliente_nombre ASC LIMIT 100";

            /*== Seleccionando clientes en la DB ==*/
            $datos_cliente=$this->ejecutarConsulta($query);
            ...
            if($datos_cliente->rowCount()>=1){
				...
				foreach($datos_cliente as $rows){
					...
					<tr style="font-size: 12px; vertical-align: middle;">
                        <td class="has-text-left">
                            <i class="fas fa-user fa-fw text-primary" style="color: #f97316;"></i> &nbsp; 
                            <strong>'.$rows['cliente_nombre'].' '.$rows['cliente_apellido'].'</strong>
                        </td>
                        <td>'.$rows['cliente_tipo_documento'].': <code>'.$rows['cliente_numero_documento'].'</code></td>
                        <td>'.$rows['cliente_telefono'].'</td>
                        <td class="has-text-centered">
                            <button type="button" class="button is-link is-rounded is-small" style="background: linear-gradient(135deg, #f97316 0%, #ff5e36 100%) !important; border:none;" onclick="agregar_cliente('.$rows['cliente_id'].')"><i class="fas fa-user-plus"></i></button>
                        </td>
                    </tr>
                    ';
				}
				...
				<article class="message is-warning mt-4 mb-4">
				    ...
				    <div class="message-body has-text-centered">
				    	<i class="fas fa-exclamation-triangle fa-2x"></i><br>
						No hemos encontrado ningún cliente en el sistema que coincida con tu búsqueda.
				    </div>
				</article>';
			}
        }
		...
		public function eliminarVentaControlador(){
		    ...
		    # Verificando detalles de venta y restaurando stock #
		    $check_detalle_venta=$this->ejecutarConsulta("SELECT * FROM venta_detalle WHERE venta_codigo='".$datos['venta_codigo']."'");
		    $detalles_venta=$check_detalle_venta->fetchAll();
		    $check_detalle_venta_count=count($detalles_venta);

		    if($check_detalle_venta_count>0){

		        # Restaurando stock de productos #
		        foreach($detalles_venta as $detalle){
		            $producto_id=$detalle['producto_id'];
		            $cantidad_vendida=$detalle['venta_detalle_cantidad'];

		            $check_producto=$this->ejecutarConsulta("SELECT producto_stock_total FROM producto WHERE producto_id='$producto_id'");
		            if($check_producto->rowCount()>0){
		                $producto_data=$check_producto->fetch();
		                $nuevo_stock=$producto_data['producto_stock_total']+$cantidad_vendida;
		                ...
		                $this->actualizarDatos("producto",$datos_producto_up,$condicion);
		            }
		        }

		        # Eliminando los detalles de venta #
		    ...
		    # Restaurando efectivo de caja #
		    $caja_id=$datos['caja_id'];
		    $venta_total=$datos['venta_total'];
		    $check_caja=$this->ejecutarConsulta("SELECT caja_efectivo FROM caja WHERE caja_id='$caja_id'");
		    if($check_caja->rowCount()>0){
		        $caja_data=$check_caja->fetch();
		        $nuevo_efectivo=$caja_data['caja_efectivo']-$venta_total;
		        if($nuevo_efectivo<0){
		            $nuevo_efectivo=0.00;
		        }
		        ...
		        $datos_caja_up=[
		            [
		                "campo_nombre"=>"caja_efectivo",
		                "campo_marcador"=>":Efectivo",
		                "campo_valor"=>$nuevo_efectivo
		            ]
		        ];
		        $condicion_caja=[
		            "condicion_campo"=>"caja_id",
		            "condicion_marcador"=>":ID",
		            "condicion_valor"=>$caja_id
		        ];
